...
async function getMentionCandidates(
  ...
  try {
    ...
    const directMatches = entries
      ...
      .map((entry) => {
        const path = directoryPart ? `${entry.name}` : entry.name;
...
export function InputBar({ onSubmit, disabled = false }: Props) {
  ...
  // Keep the file picker in sync with the current @mention toke.
  ...
  useKeyboard((key) => {
    ...
    if (key.name === "escape") {
      ...
      setMentionSelectedIndex((currentIndex) => {
        ...
        if (scrollbox) {
          const viewposrtHeight = scrollbox.viewport.height;
          const visibleEnd = scrollbox.scrollTop + viewposrtHeight - 1;
          if (nextIndex > visibleEnd) {
            scrollbox.scrollTo(nextIndex - viewposrtHeight + 1);
          }
        }
